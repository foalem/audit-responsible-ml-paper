...
def get_repo_info(repo_list: List[str]) -> tuple[list[Any], list[Any], list[Any], list[Any]]:
    """
    ...
    return stars_count_list, created_date_list, description_list, language_list
...
def save_metric(rep_list_: List[str], contrib_list: List[int], star_list: List[int], created_list: List[str],
                descript_list: List[str], lang_list: List[str], com_list: List[int]) -> None:
    headers = ['repo_name', 'number_contributor', 'number_stars', 'created_date',
               'description', 'language', 'number_commit']
    rows = []
    for i in range(len(star_list)):
        row = [rep_list_[i], contrib_list[i], star_list[i],
               created_list[i], descript_list[i], lang_list[i], com_list[i]]
        rows.append(row)
