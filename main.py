...
from data__collection.get_issue import get_repository_issues, save_issues_to_csv
...
def main() -> None:
    ...
    parser.add_argument('-i', '--issue', help="Collect issue from repository", dest='ISSUE', required=False,
                        action='store_true')
    ...
    if args.METRIC:
        ...
        stars_list, created_date_list_, description_list_, language_list_ = get_repo_info(repo_list_)
        save_metric(repo_list_, contributor_list, stars_list, created_date_list_,
                    description_list_, language_list_, commits_count_list_)
    ...
    if args.PARSER:
        ...
        filter_libs(libraries_to_track, all_call_function,
                    ROOT_DIR + '/' + PATH_FILE['data'] + 'call_function_logging_.csv')
        # __function_call = find_library_function_calls(f"{ROOT_DIR}{PATH_FILE['data']}clones", lib_)
        # print(__function_call)
    if args.ISSUE:
        repo_list = get_distinct_repo_names(f"{ROOT_DIR}{PATH_FILE['data']}ML_project_dataset.csv")
